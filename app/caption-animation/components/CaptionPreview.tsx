...
export const CaptionPreview = () => {
  const { lines, currentLine, background } = useCaptionStore();
  ...
  if (background.type === 'gradient') {
    ...
    const svg = encodeURIComponent(`<svg width='${spacing}' height='${spacing}' viewBox='0 0 ${spacing} ${spacing}' fill='none' xmlns='http://www.w3.org/2000/svg'><circle cx='${spacing/2}' cy='${spacing/2}' r='${dotSize}' fill='${dotColor}'/></svg>`);
  ...
  return (
    <div
      className="w-full h-64 flex items-center justify-center rounded shadow relative"
      style={bgStyle}
    >
      {line && (
        <div
